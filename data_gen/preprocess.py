...
from utils import get_sample_index
def sample_pre_normalization(data, zaxis=[0, 1], xaxis=[8, 4],num_sample=48):
    N, C, T, V, M = data.shape
    s = np.transpose(data, [0, 4, 2, 3, 1])  # N, C, T, V, M  to  N, M, T, V, C
        ...
        for i_p, person in enumerate(skeleton):
            ...
            for i_f, frame in enumerate(person):
                if frame.sum() == 0:
                    if person[i_f:].sum() == 0:
                        effectframe = i_f
                        if(effectframe < num_sample):
                            # print(effectframe) #the number of the condition is very little , you also can discard these
                            rest = num_sample - i_f
                            num = int(np.ceil(rest / i_f))
                            pad = np.concatenate([person[0:i_f] for _ in range(num)], 0)[:rest]
                            s[i_s, i_p, i_f:num_sample] = pad
                            effectframe = num_sample
                        index = get_sample_index(effectframe,num_sample)
                        s[i_s, i_p, :num_sample] = s[i_s, i_p, index]
                        break
...
if __name__ == '__main__':
    data = np.load('../data/ntu_subsample/xsub/val_data_joint_origin.npy')
    sample_pre_normalization(data)
    np.save('../data/ntu/xview/data_val_sample_pre.npy', data)
